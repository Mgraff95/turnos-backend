 ...
 *
 * Respeta `max_simultaneos` del ancla (cuántos compatibles puede llevar en paralelo) y
 * `intercalar_desde_min` (a partir de qué minuto del ancla puede arrancar el compatible).
 *
 * Devuelve:
...
function resolverBloqueConIntercalados(items) {
  ...
  items.forEach(it => {
    ...
    const maxSimult = (it.servicio.max_simultaneos || 2) - 1; // cuántos compatibles admite en paralelo
    items.forEach(otro => {
      ...
      if (yaAsignados >= maxSimult) return; // se pasó del máximo simultáneo, queda como secuencial
