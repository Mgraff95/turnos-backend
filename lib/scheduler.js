...
function iniciarScheduler() {
  ...
  // Reconciliación: pagos aprobados cuyo webhook nunca llegó.
  // El webhook es el camino normal, pero puede perderse (caída nuestra, red,
  // reintentos agotados). Sin esto, la clienta pagó y no tiene turno, y nadie
  // se entera hasta que reclama.
  cron.schedule('*/15 * * * *', async () => {
    try {
      await reconciliarPagos();
    } catch (error) {
      console.error('❌ Error en la reconciliación de pagos:', error.message);
    }
  });
  ...
  console.log('🔁 Reconciliación de pagos iniciada (cada 15 min)');
}
 
// ── Reconciliar pagos cuyo webhook no llegó ────────────
//
// Recorre las reservas de las últimas 24 h que quedaron sin completarse y le
// pregunta a Mercado Pago si existe un pago aprobado para esa referencia. Si
// existe, lo procesa como si hubiera llegado el webhook (procesarPagoAprobado
// es idempotente, así que no hay riesgo de duplicar nada).
async function reconciliarPagos() {
  const { estaConfigurado, buscarPagosPorReferencia } = require('../services/mercadopago');
  if (!estaConfigurado()) return 0;

  const { procesarPagoAprobado } = require('./pagos-proceso');
  const desde = new Date(Date.now() - 24 * 60 * 60 * 1000);

  const candidatas = await prisma.reservaPendiente.findMany({
    where: {
      estado: { in: ['activa', 'expirada', 'cancelada'] },
  ...
  for (const reserva of candidatas) {
    try {
      const pagos = await buscarPagosPorReferencia(reserva.external_ref);
      const aprobado = pagos.find(p => p.status === 'approved');
      if (!aprobado) continue;

      console.log(`🔁 Reconciliando pago ${aprobado.id} (${reserva.external_ref}): el webhook no había llegado`);
      await procesarPagoAprobado(aprobado);
      recuperados++;
    } catch (error) {
      console.error(`   ❌ Error reconciliando ${reserva.external_ref}:`, error.message);
    }
  }

  if (recuperados > 0) console.log(`   ✅ ${recuperados} pago(s) recuperado(s) por reconciliación`);
  return recuperados;
}
...
module.exports = { iniciarScheduler, enviarRecordatoriosPendientes, limpiarReservasVencidas, reconciliarPagos };
