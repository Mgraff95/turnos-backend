...
const { enviarRecordatorio, enviarRecordatorioGrupo } = require('../services/whatsapp');
...
async function enviarRecordatoriosPendientes() {
  ...
  // Filtrar solo los que están efectivamente dentro de la ventana de 2-26h
  const enVentana = turnos.filter(turno => {
    const fechaTurno = new Date(
      `${turno.fecha.toISOString().split('T')[0]}T${turno.hora_inicio}:00`
    );
    const horasHastaTurno = (fechaTurno - ahora) / (1000 * 60 * 60);
    return horasHastaTurno >= 2 && horasHastaTurno <= 26;
  });
 
  if (enVentana.length === 0) {
    console.log('   No hay recordatorios pendientes dentro de la ventana');
    return;
  }
 
  // Agrupar por grupo_reserva: los turnos de un mismo bloque (reserva de varios
  // servicios) van todos en UN SOLO mensaje, en vez de uno por servicio.
  const sueltos = enVentana.filter(t => !t.grupo_reserva);
  const gruposMap = {};
  enVentana.filter(t => t.grupo_reserva).forEach(t => {
    (gruposMap[t.grupo_reserva] = gruposMap[t.grupo_reserva] || []).push(t);
  });
 
  console.log(`   Enviando ${sueltos.length} recordatorio(s) individual(es) y ${Object.keys(gruposMap).length} de grupo...`);
 
  // Turnos sueltos: un mensaje por turno (comportamiento original)
  for (const turno of sueltos) {
  ...
  // Turnos agrupados: un solo mensaje por grupo_reserva
  for (const grupoId in gruposMap) {
    const turnosGrupo = gruposMap[grupoId].sort((a, b) => a.hora_inicio.localeCompare(b.hora_inicio));
    const enviado = await enviarRecordatorioGrupo(turnosGrupo);
    if (enviado) {
      await prisma.turno.updateMany({
        where: { id: { in: turnosGrupo.map(t => t.id) } },
        data: { recordatorio_enviado: true }
      });
      console.log(`   ✅ Recordatorio de grupo enviado a ${turnosGrupo[0].cliente_nombre} (${turnosGrupo.length} servicios)`);
    }
  }
}
